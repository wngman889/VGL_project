            ...
            $.each(data, function (index, event) {
                getGameDetails(event.gameId, function (gameDetails) {
                    getUserDetails(event.authorId, function (userDetails) {
                        ...
                        if (event.EventImageBase64) {
                            var img = $("<img>").addClass("latest-image").attr("src", "data:image/png;base64," + event.EventImageBase64).attr("alt", "Event Image").on("error", function() {
                                // Handle image load errors
                                $(this).attr("src", "imgs/misc/default-image.png"); // Provide a fallback image path
                            });
